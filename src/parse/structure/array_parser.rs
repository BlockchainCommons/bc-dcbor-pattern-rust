        ...
        Some(Ok(Token::ParenOpen)) => {
            ...
            match second_lookahead.next() {
                Some(Ok(Token::Range(res))) => {
                    // This is a quantifier syntax: ARRAY({n}), ARRAY({n,m}), etc.
                    let quantifier = res?;
...
mod tests {
    ...
    fn test_array_pattern_matching() {
        ...
        let pattern = Pattern::Structure(crate::pattern::StructurePattern::Array(
            ArrayPattern::with_elements(Pattern::number(42))
        ));

        // Should match array with exactly one element: 42
        let cbor_single = parse_dcbor_item("[42]").unwrap();
        assert!(pattern.matches(&cbor_single), "ARRAY(NUMBER(42)) should match [42]");

        // Should NOT match array with multiple elements including 42
        let cbor_multi = parse_dcbor_item("[1, 42, 3]").unwrap();
        assert!(!pattern.matches(&cbor_multi), "ARRAY(NUMBER(42)) should NOT match [1, 42, 3]");

        // Should NOT match array without 42
        let cbor_no_match = parse_dcbor_item("[1, 2, 3]").unwrap();
        assert!(!pattern.matches(&cbor_no_match), "ARRAY(NUMBER(42)) should NOT match [1, 2, 3]");

        // Should NOT match empty array
        let cbor_empty = parse_dcbor_item("[]").unwrap();
        assert!(!pattern.matches(&cbor_empty), "ARRAY(NUMBER(42)) should NOT match []");

        // Test 2: Sequence matching
        let sequence_pattern = Pattern::Structure(crate::pattern::StructurePattern::Array(
        ...
        assert!(sequence_pattern.matches(&cbor_seq_match), r#"ARRAY(TEXT("a") > TEXT("b")) should match ["a", "b"]"#);

        // Should NOT match wrong order
        let cbor_seq_wrong = parse_dcbor_item(r#"["b", "a"]"#).unwrap();
        assert!(!sequence_pattern.matches(&cbor_seq_wrong), r#"ARRAY(TEXT("a") > TEXT("b")) should NOT match ["b", "a"]"#);

        // Should NOT match partial sequence
        let cbor_seq_partial = parse_dcbor_item(r#"["a"]"#).unwrap();
        assert!(!sequence_pattern.matches(&cbor_seq_partial), r#"ARRAY(TEXT("a") > TEXT("b")) should NOT match ["a"]"#);

        // Should NOT match longer sequence
        let cbor_seq_long = parse_dcbor_item(r#"["a", "b", "c"]"#).unwrap();
        assert!(!sequence_pattern.matches(&cbor_seq_long), r#"ARRAY(TEXT("a") > TEXT("b")) should NOT match ["a", "b", "c"]"#);

        // Test 3: Or pattern (should still work with individual element matching)
        let or_pattern = Pattern::Structure(crate::pattern::StructurePattern::Array(
            ArrayPattern::with_elements(Pattern::or(vec![Pattern::any_number(), Pattern::any_text()]))
        ));
